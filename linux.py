...
import json, csv, os, time, random, sys, traceback, pickle, requests
from selenium.webdriver.firefox.firefox_profile import FirefoxProfile

URL = "https://instagram.com/{}/" ## Instagram URL
SOCKSPort = 9050 ## Tor SOCKS listner port
ControlPort = 9051 ## Tor control control port
...
def check_tor_conn():
    title = str(bs(requests.get("https://check.torproject.org", proxies = proxies).text, "html.parser").title).split("\n")[2]
    if "Congratulations." in title: print("\nTor Connectivity Verified"), slack.post(text = "\nTor Connectivity Verified")
    elif "not using Tor" in title: print("\nTor is not Connected, Try 'sudo service tor restart'. Aborting..."), slack.post(text = "\nTor is not Connected, Try 'sudo service tor restart'. Aborting..."), sys.exit(0)
check_tor_conn()
...
def scrape_data(username):
    ...
    return parse_data(bs(page_source, "html.parser"))
...
def check_csv_file(data, filename):
    ...
    if os.path.isfile(OUTPUT_FOLDER + filename):
        with open(OUTPUT_FOLDER + filename, "r") as rf:
            ...
            for key_index in specific_key_index: true_false_list.append(data[list(data.keys())[key_index]] == last_line_items[key_index])
    ...
    ## This block will create an csv file if you are runnig this script first time.
        ...
        with open(OUTPUT_FOLDER + filename, "a", newline = "") as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(list(data.keys())), csvwriter.writerow([data[keys] for keys in data.keys()])
...
def random_sleep_time(secends): return random.randint(secends - (secends * 0.3), secends + (secends * 0.4))
...
def ig_login(email, password):
    print(f"\nTrying to logging in using {email}..."), slack.post(text = f"Trying to logging in using {email}"), browser.get("https://instagram.com"), browser.execute_script("window.scrollTo(0,document.body.scrollHeight)"), time.sleep(2.6), browser.execute_script("window.scrollTo(document.body.scrollHeight, 0)"), time.sleep(0.6)
    a = browser.find_element_by_xpath('//*[@id="loginForm"]/div/div[1]/div/label/input')
    a.click(), time.sleep(3.7)
    for words in email: a.send_keys(words), time.sleep(0.3)
    b = browser.find_element_by_xpath('//*[@id="loginForm"]/div/div[2]/div/label/input')
    b.click(), time.sleep(1.6)
    for words in password: b.send_keys(words), time.sleep(0.25)
    browser.find_element_by_xpath('//*[@id="loginForm"]/div/div[3]/button/div').click(), print(f"\nSuccessfully logged in {email}..."), slack.post(text = f"Successfully logged in {email}"), time.sleep(20)
    try: browser.find_element_by_xpath('//*[@id="react-root"]/section/main/div/div/div/div/button').click(), time.sleep(15), browser.find_element_by_xpath('/html/body/div[4]/div/div/div/div[3]/button[2]').click(), time.sleep(15)
    except: pass

## i'm currently working on this function, maybe you'll see it in upcoming commits/versions.
def specific_user_aleart(data): pass
...
def save_ss(event = None):
    ...
    if not t1 or ((t2-t1)>120):# 2Minutes
        ...
        browser.get_screenshot_as_file(f'./ss_log/browser/{now.strftime("%d_%m_%Y")}__{now.strftime("%H_%M")}.png'), print(f'\nBrowser Screenshot saved at ./ss_log/browser/{now.strftime("%d_%m_%Y")}__{now.strftime("%H_%M")}.png')
...
def get_tor_ip():
    try: return requests.get("http://httpbin.org/ip", proxies = proxies).text.split('"')[3] ## return ip as string
...
def renew_tor_ip():
    ...
    with Controller.from_port(port = ControlPort) as controller:
        controller.authenticate(), controller.signal(Signal.NEWNYM) ## input authentication password as string.


## This Function will log out your id from browser, after logout succeed all cookies will be deleted so we can login new id.
def ig_logout(email): browser.get("https://instagram.com"), print(f"\nTrying to logging out {email}..."), slack.post(text = f"Trying to logging out {email}"), browser.find_element_by_xpath('//*[@id="react-root"]/section/nav/div[2]/div/div/div[3]/div/div[5]/span').click(), time.sleep(1), browser.find_element_by_xpath('//div[text() = "Log Out"]').click(), browser.delete_all_cookies(), print(f"\nSuccessfully logged out {email}"), slack.post(text = f"Successfully logged out {email}")
...
while True: ## To keep running this scraper.
    for username in usernames: ## load a instagram username from usernames list, which we have loaded previously
        ...
        try:
            ...
            check_csv_file(data, filename), print("\n", username, " ", data)
            sleep_time = random_sleep_time(120)# 2 Minutes
            print(f"\nsleeping for {sleep_time} secends....."), time.sleep(sleep_time)
        ...
        except KeyboardInterrupt: print("\n^C Detected. Taking you to exit options...."), exit_options()
        except Exception as e:
            exception = traceback.format_exc() ## to format exceptions
            print(f"\nException Occurred while scraping for {username}", str(exception)), save_ss(), slack_hook(username = username, exception = exception)